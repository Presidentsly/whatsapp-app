...
// Biztonságos LocalAuth hely Render-en
...
wss.on('connection', socket => {
    ...
    socket.on('message', async data => {
        ...
        } catch(err){ console.error('WS hiba:', err); }
    });
});

// Bejövő WhatsApp üzenetek kezelése
client.on('message', async msg => {
    try{
        const item = { from: msg.from, name: msg._data?.notifyName || msg.from, text: msg.body, t: Date.now() };
        if(msg.hasMedia){
            const media = await msg.downloadMedia();
            if(media && media.data) item.media = { mimetype: media.mimetype, data: media.data };
        }
        if(!item.media){
            messages.push(item);
            if(messages.length > 200) messages.shift();
    ...
    } catch(err){ console.error('WhatsApp hiba:', err); }
});

// Frontend HTML + WebSocket + emoji
app.get('/', (req,res)=>{
    res.setHeader('Content-Type','text/html; charset=utf-8');
    res.send(`<!doctype html>
<html lang="hu">
<head><meta charset="utf-8"><title>WhatsApp Közös Nézet + Válasz</title>
<style>
body{font-family:sans-serif;background:#f4f4f4;}
.header{display:flex;justify-content:space-between;align-items:center;text-align:center;margin:10px auto;font-size:18px;color:#000;max-width:800px;}
...
<body>
<div class="header"><div>WhatsApp – Közös nézet és válasz</div><div class="clock" id="clock">--:--:--</div></div>
...
function addMessage(msg){const wrap=document.createElement('div');wrap.className='msg';wrap.dataset.from=msg.from;const meta=document.createElement('div');meta.className='meta';meta.textContent=(msg.name||msg.from)+' @ '+new Date(msg.t).toLocaleString();const replyBtn=document.createElement('button');replyBtn.textContent='Válasz';replyBtn.className='reply-btn';replyBtn.onclick=()=>targetInput.value=msg.from;meta.appendChild(replyBtn);const delBtn=document.createElement('button');delBtn.textContent='Törlés';delBtn.className='del-btn';delBtn.onclick=()=>wrap.remove();meta.appendChild(delBtn);wrap.appendChild(meta);if(msg.text)wrap.appendChild(document.createTextNode(msg.text));if(msg.media&&msg.media.data){const img=document.createElement('img');img.src="data:"+msg.media.mimetype+";base64,"+msg.media.data;img.className="media";wrap.appendChild(img);}messagesEl.appendChild(wrap);messagesEl.scrollTop=messagesEl.scrollHeight;}
