...
.msg .info { width:180px; font-size:12px; color:#666; margin-right:10px; white-space: pre-line; }
...
function addMessage(msg){
    ...
    if(msg.media && msg.media.data){
        ...
        img.className='media'; content.appendChild(img);
    }
    const replyBtn=document.createElement('button'); replyBtn.textContent='Válasz';
    replyBtn.className='reply-btn'; replyBtn.onclick=()=>targetInput.value=msg.from;
    const delBtn=document.createElement('button'); delBtn.textContent='Törlés';
    delBtn.className='del-btn'; delBtn.onclick=()=>wrap.remove();
    content.appendChild(replyBtn); content.appendChild(delBtn);
    wrap.appendChild(info); wrap.appendChild(content);
...
for(const cat in emojiCategories){
    emojiCategories[cat].forEach(e=>{
        ...
        btn.textContent=e; btn.className='emoji-btn';
